...
#    print 'sa :', self.array_suffix
#    print 'tmp :', tmp
#    assert(self.array_suffix == tmp)
    ...
    for j in xrange(k) :
      ...
      if rank[j] != 0 :
        ...
        str_j = self.array_str[id_str_j]
        len_j = len(str_j) - o_j
        o_jj, id_str_jj  = self.array_suffix[SA[rank[j]-1]]
        str_jj = self.array_str[id_str_jj]
        len_jj = len(str_jj) - o_jj
        while(l < len_j and l < len_jj and str_j[l + o_j] == str_jj[l + o_jj]) :
          l += 1
        lcp[rank[j]-1] = l
      else :
        l = 0
...
#      offset2, _, idStr2  = self.array_suffix[idx+1]
      o1, id_str1  = self.array_suffix[idx]
      o2, id_str2  = self.array_suffix[idx+1]
      end_ = max((id_str1, o1+current_len), (id_str2, o2+current_len)) 
